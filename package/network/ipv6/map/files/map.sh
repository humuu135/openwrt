...
proto_map_setup() {
	...
	json_add_array firewall
	  if [ -z "$(eval "echo \$RULE_${k}_PORTSETS")" ]; then
	    ...
	    /sbin/sysctl -w kernel.nf_nat.nf_nat_psid=$(eval "echo \$RULE_${k}_PSID")
	    /sbin/sysctl -w kernel.nf_nat.nf_nat_psid_mask=$(eval "echo \$RULE_${k}_PSID_MASK")
	    /sbin/sysctl -w kernel.nf_nat.nf_nat_psid_ip_min=$(eval "echo \$RULE_${k}_PSID_IP")
	    /sbin/sysctl -w kernel.nf_nat.nf_nat_psid_ip_max=$(eval "echo \$RULE_${k}_PSID_IP")
